    ...
    eventSource.addEventListener('sentiment', (event) => {
      try {
        ...
        setSentimentHistory((prev) => {
          const key = `${data.window_start}-${data.window_end}`;
          const map = new Map(prev.map(item => [`${item.window_start}-${item.window_end}`, item]));

          map.set(key, data); // overwrite old value for this window
